...
public record EventResponse(UUID id, String title, String description, String location,
                            ...
                            boolean registeredByCurrentUser, long attendeeCount) {
    static EventResponse from(EventEntity event, boolean registered, long attendeeCount) {
        return new EventResponse(event.getId(), event.getTitle(), event.getDescription(), event.getLocation(),
                event.getStartsAt(), event.getEndsAt(), event.getStatus(), registered, attendeeCount);
    }
}
